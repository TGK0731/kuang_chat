...
const ContextProvider = (props) => {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:3001/api/rag/documents');
      const data = await res.json();
      setRagDocs(data.documents || []);
      if (data.documents?.length === 0) setIsRagActive(false);
    } catch (e) {
  ...
  const removeDocument = useCallback(async (filename) => {
    await fetch('http://localhost:3001/api/rag/documents', {
      method: 'POST',
